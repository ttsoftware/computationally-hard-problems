...
$A_d$ takes the set of possible moves $S = \{s_0, s_1, ...,s_i\}$, where $i = (m+n) \cdot \frac{(m-1) + (n-1)}{2}$, and creates a set $R = \{R_0, R_1, ..., R_j\}$, containing every combination of $k$ moves from $S$. As there exist $i$ such moves, we will have $j = i^k$.
...
In the worst case, $A_d$ creates $j = i^k$ sets, where $i = ((m+n) \cdot \frac{(m-1) + (n-1)}{2})$. Each set can be created and checked in constant time. This gives $A_d$ a worst-case running time of $T(A_d) = O(i^k)$.

$A_o$ calls $A_d$ until it has found the optimal value of $k$, which takes $k$ calls. When $k$, and thus the solution $R_0$ has been found, the algorithm terminates, which gives us a running time of $T(A_o) = O(k T(A_d)) = O(k(i^k)$
